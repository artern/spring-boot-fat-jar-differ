...
项目当前发布在 `io.github.artern.spring-boot-fat-jar-differ` 命名空间下，提供三类产物：
...
## 迁移说明：group 坐标变更（0.1.2）

从版本 `0.1.2` 起，Maven group ID 已变更：
...
| Gradle 插件 jar | `io.github.artern:gradle-plugin` | `io.github.artern.spring-boot-fat-jar-differ:differ-gradle-plugin` |
| Plugin ID | `io.github.artern.spring-boot-fat-jar-differ` | 不变 |

Gradle plugin ID（`io.github.artern.spring-boot-fat-jar-differ`）及其自动生成的 plugin marker artifact 保持不变。只需更新 buildscript 的 `classpath` 依赖以及直接引用 Maven 坐标的地方。
...
对于仍使用老式 `buildscript` 方式的可执行 WAR 工程，可以在 `buildscript` classpath 中加入 `io.github.artern.spring-boot-fat-jar-differ:differ-gradle-plugin:0.1.2`，然后再 `apply plugin: 'io.github.artern.spring-boot-fat-jar-differ'`。
...
1. baseline 缺失时明确失败。
2. 从 `mavenLocal()` 解析 `io.github.artern.spring-boot-fat-jar-differ:differ-cli:0.1.2`。
